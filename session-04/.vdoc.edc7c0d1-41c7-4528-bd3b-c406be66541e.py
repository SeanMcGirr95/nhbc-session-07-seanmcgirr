...
ex5_dates = (
    ...
    .select(
        ...
        fiscal_quarter_to_date_udf(f.col("FIRSTNOTIFIEDFYQUARTER")).alias("First Notified"),
        f.col("PROCESSINGQUARTER"),
        fiscal_quarter_to_date_udf(f.col("PROCESSINGQUARTER")).alias("Processed Date")
        )
)
...
def get_aggregated_data(category, uwy_start, seg_class):
    ...
    agg_data = (
        claims_data
        .select(f.col("ReservingSegmentationGroup_Excess"),f.col("UWY"))
        .group_by(f.col("ReservingSegmentationGroup_Excess").alias('"Groups"'))
        .agg(f.col("ReservingSegmentationGroup_Excess").alias('"Groups"'))
    )
    # Display Snowpark DataFrame
    agg_data.show()  # Shows the result in tabular format  
...
#
#
from snowflake.snowpark import functions as ffrom snowflake.snow.
    Returns a Snowpark DataFrame with Groups and ClaimCount.
    """
    agg_data = (
        claims_data
        .filter(f.col("Category") == category)              # Filter by category
        .filter(f.col("UWY") >= uwy_start)                 # Filter by UWY start
        .filter(f.col("SegmentationClass") == seg_class)   # Filter by segmentation class
        .group_by(f.col("ReservingSegmentationGroup_Excess").alias('"Groups"'))
        .agg(f.count("*").alias("ClaimCount"))             # Aggregate count of claims
        .sort("ClaimCount", ascending=False)               # Sort by count descending
    )

    agg_data.show()  # Display the result
    return agg_data

def get_aggregated_data(category: str, uwy_start: int, seg_class: str):
    """
    Aggregate claims data by ReservingSegmentationGroup_Excess for a given category,
